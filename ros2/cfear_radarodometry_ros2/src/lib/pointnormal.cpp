...
MapPointNormal::MapPointNormal(const pcl::PointCloud<pcl::PointXYZI>::Ptr& cld, float radius, const Eigen::Vector2d& origin, const bool weight_intensity, const bool raw) : weight_intensity_(weight_intensity)  {
  ...
  if(input_->size()==0){
    cerr<<"error, cloud empty - no normals computed"<<endl;
    return; // online: an (almost) empty scan must not kill the node; cells stay empty
  }
  if(raw){
...
// Visualization (rviz marker publishing) stripped in the ROS2 port: the ROS1
// implementation lazily created publishers from a global NodeHandle inside these
// static methods, which has no rclcpp equivalent. See the ROS1 tree to restore it.
...
void MapPointNormal::PublishDataAssociationsMap(const std::string& topic, const std::vector<std::tuple<Eigen::Vector2d,Eigen::Vector2d,double,int> >& vis_residuals){
  (void)topic; (void)vis_residuals; // visualization stripped in the ROS2 port
}
...
void MapPointNormal::PublishMap(const std::string& topic, MapNormalPtr map, const Eigen::Affine3d& T, const std::string& frame_id, const int value, float alpha){
  (void)topic; (void)map; (void)T; (void)frame_id; (void)value; (void)alpha; // visualization stripped in the ROS2 port
}
