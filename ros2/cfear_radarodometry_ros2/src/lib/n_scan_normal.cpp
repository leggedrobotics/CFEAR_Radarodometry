...
//return success;
  ...
  if(success){
    ...
    // A rank-deficient covariance (degenerate geometry) must NOT invalidate the
    // pose solution itself - the default diagonal set above stays in place.
    // (ROS1 returned false here, but a variable-shadowing bug in the fuser
    // ignored it and used the pose anyway; we keep that effective behavior.)
    if(!GetCovariance(reg_cov.back()))
      cerr<<"covariance estimation failed (rank-deficient) - keeping default covariance"<<endl;
  ...
  for(size_t src_idx=0 ; src_idx<src_local->GetSize() ; src_idx++){
    if(time_continuous_){ // Actually doesn't improve the results
      if (scan_idx_src != parameters.size()-1) {
        throw std::logic_error("n_scan_normal_reg::AddScanPairCost: time-continuous src index mismatch");
      }
...
bool n_scan_normal_reg:: BuildOptimizationProblem(std::vector<MapNormalPtr>& scans, const Eigen::MatrixXd& cov, const Eigen::Vector3d& guess, bool soft_constraints){
  ...
  if(fixedBlock_.size()==0)
    ...
    cerr<<"Fixed Block size error - skipping this registration"<<endl;
    return false; // online: recoverable, the caller falls back to the motion prediction
  }
  return true;
}
